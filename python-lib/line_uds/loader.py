    ...
    with open(profile, 'r') as f:
        ...
        for (name, prop) in data['properties'].items():
            if prop['type'] in ['uint8', 'uint16', 'uint32', 'uint64']:
                ...
                properties.append(UdsNumericProperty(name, int(prop['id'], 0), byte_size, False))
            if prop['type'] in ['int8', 'int16', 'int32', 'int64']:
                byte_size = int(prop['type'][3:]) // 8
                properties.append(UdsNumericProperty(name, int(prop['id'], 0), byte_size, True))
            elif prop['type'] == 'bool':
                properties.append(UdsBooleanProperty(name, int(prop['id'], 0)))
            elif prop['type'] == 'enum':
                properties.append(UdsEnumProperty(name, int(prop['id'], 0), prop['values']))
